...
export default function AdminEpisodesPage() {
  ...
  const [form, setForm] = useState({
    title: "", description: "", episodeNumber: "", seasonNumber: "1",
    duration: "", thumbnailUrl: "", videoUrl: "", hlsUrl: "", seriesId: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (d.success) {
        ...
        setShowForm(false);
        const e2 = await fetch("/api/episodes").then(r => r.json());
        if (e2.success) setEpisodes(e2.data);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => setShowForm(!showForm)}><Plus className="h-4 w-4 mr-1" />Bölüm Ekle</Button>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm text-white">Dizi *</label>
              ...
              <div className="space-y-2">
                <label className="text-sm text-white">Bölüm Adı *</label>
                <Input value={form.title} onChange={(e) => setForm(f => ({ ...f, title: e.target.value }))} required />
              </div>
              <div className="space-y-2">
                <label className="text-sm text-white">Bölüm No *</label>
                <Input value={form.episodeNumber} onChange={(e) => setForm(f => ({ ...f, episodeNumber: e.target.value }))} type="number" required />
              </div>
              <div className="space-y-2">
                <label className="text-sm text-white">Sezon No</label>
                <Input value={form.seasonNumber} onChange={(e) => setForm(f => ({ ...f, seasonNumber: e.target.value }))} type="number" />
              </div>
              <div className="space-y-2">
                <label className="text-sm text-white">Süre (dk)</label>
                <Input value={form.duration} onChange={(e) => setForm(f => ({ ...f, duration: e.target.value }))} type="number" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input value={form.hlsUrl} onChange={(e) => setForm(f => ({ ...f, hlsUrl: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <label className="text-sm text-white">Thumbnail URL</label>
            ...
            <Button type="submit" disabled={submitting}>
              {submitting ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}Ekle
            </Button>
          </form>
        </div>
      )}
            ...
            <tbody>
              {filtered.map((ep: any) => (
                <tr key={ep.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  <td className="p-4">
                    <p className="text-white">{ep.title}</p>
                  </td>
